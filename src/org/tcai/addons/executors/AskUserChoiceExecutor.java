...
public class AskUserChoiceExecutor extends BaseActionExecutor {
	...
	protected void internalExecute() {
		...
		builder.setSingleChoiceItems(
				mAddonAction.getChoices().toArray(
						new String[mAddonAction.getChoices().size()]), 0,
				new OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						...
						Controller
								.getInstance()
								.getAddonManager()
								.onUserChoice(mContext, mWebView, mAddon,
										mAddonAction.getId(), false, which);
					}
				});
