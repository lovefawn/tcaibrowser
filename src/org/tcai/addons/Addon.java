...
public class Addon {
	...
	public Addon(Context context, int id, ResolveInfo resolveInfo,
			String category) {
		mContext = context;
		mId = id;
		...
		mServiceConnection
				.setAddonServiceConnectionListener(new AddonServiceConnectionListener() {
					@Override
	...
	/**
	 * Gets the menu id for this addon. I'm not sure how to avoid conflicts
	 * between Android generated menu ids, and the ones used when an addon
	 * contribute to a menu. For now, make the assumption that Android do not
	 * use negatives ids, and start from - 1000. The real addon id can be
	 * retrived by the inverse operation.
	 * 
	...
	public String getContributedMainMenuItem(String currentTabId,
			String currentTitle, String currentUrl) {
		if (makeCall(Callbacks.CONTRIBUTE_MAIN_MENU)) {
			return mServiceConnection.getContributedMainMenuItem(currentTabId,
	...
	public List<Action> onContributedMainMenuItemSelected(String currentTabId,
			String currentTitle, String currentUrl) {
		if (makeCall(Callbacks.CONTRIBUTE_MAIN_MENU)) {
			return mServiceConnection.onContributedMainMenuItemSelected(
					currentTabId, currentTitle, currentUrl);
		} else {
			return null;
		}
	}

	public String getContributedLinkContextMenuItem(String currentTabId,
			int hitTestResult, String url) {
		if (makeCall(Callbacks.CONTRIBUTE_LINK_CONTEXT_MENU)) {
			return mServiceConnection.getContributedLinkContextMenuItem(
					currentTabId, hitTestResult, url);
		} else {
			return null;
		}
	}

	public List<Action> onContributedLinkContextMenuItemSelected(
			String currentTabId, int hitTestResult, String url) {
		if (makeCall(Callbacks.CONTRIBUTE_LINK_CONTEXT_MENU)) {
			return mServiceConnection.onContributedLinkContextMenuItemSelected(
					currentTabId, hitTestResult, url);
		} else {
			return null;
		}
	...
	public String getContributedHistoryBookmarksMenuItem(String currentTabId) {
		if (makeCall(Callbacks.CONTRIBUTE_HISTORY_BOOKMARKS_MENU)) {
			return mServiceConnection
					.getContributedHistoryBookmarksMenuItem(currentTabId);
		} else {
			return null;
		}
	}
	...
	public List<Action> onContributedHistoryBookmarksMenuItemSelected(
			String currentTabId) {
		return mServiceConnection
				.onContributedHistoryBookmarksMenuItemSelected(currentTabId);
	}
	...
	public String getContributedBookmarkContextMenuItem(String currentTabId) {
		if (makeCall(Callbacks.CONTRIBUTE_BOOKMARK_CONTEXT_MENU)) {
			return mServiceConnection
					.getContributedBookmarkContextMenuItem(currentTabId);
		} else {
			return null;
		}
	}
	...
	public List<Action> onContributedBookmarkContextMenuItemSelected(
			String currentTabId, String title, String url) {
		if (makeCall(Callbacks.CONTRIBUTE_BOOKMARK_CONTEXT_MENU)) {
			return mServiceConnection
					.onContributedBookmarkContextMenuItemSelected(currentTabId,
							title, url);
		} else {
			return null;
		}
	...
	public String getContributedHistoryContextMenuItem(String currentTabId) {
		if (makeCall(Callbacks.CONTRIBUTE_HISTORY_CONTEXT_MENU)) {
			return mServiceConnection
					.getContributedHistoryContextMenuItem(currentTabId);
		} else {
			return null;
		}
	}
	...
	public List<Action> onContributedHistoryContextMenuItemSelected(
		...
		if (makeCall(Callbacks.CONTRIBUTE_HISTORY_CONTEXT_MENU)) {
			return mServiceConnection
					.onContributedHistoryContextMenuItemSelected(currentTabId,
							title, url);
		} else {
			return null;
		}
	...
	public List<Action> onUserConfirm(String currentTabId, int questionId,
			boolean positiveAnswer) {
		if (makeCallWithoutSpecificCallback()) {
			return mServiceConnection.onUserConfirm(currentTabId, questionId,
					positiveAnswer);
		} else {
			return null;
		}
	...
	public List<Action> onUserInput(String currentTabId, int questionId,
			boolean cancelled, String userInput) {
		if (makeCallWithoutSpecificCallback()) {
			return mServiceConnection.onUserInput(currentTabId, questionId,
					cancelled, userInput);
		} else {
			return null;
		}
	...
	public List<Action> onUserChoice(String currentTabId, int questionId,
			boolean cancelled, int userChoice) {
		if (makeCallWithoutSpecificCallback()) {
			return mServiceConnection.onUserChoice(currentTabId, questionId,
					cancelled, userChoice);
		} else {
			return null;
		}
	}
	...
	private void init() {
		...
		if (!TextUtils.isEmpty(mName)) {
			mPreferenceName = Constants.TECHNICAL_PREFERENCE_ADDON_ENABLED
					+ mName.toUpperCase().replace(" ", "_");
			mIsEnabled = PreferenceManager
					.getDefaultSharedPreferences(mContext).getBoolean(
							mPreferenceName, true);
				...
				public void onSharedPreferenceChanged(
					...
					if (mPreferenceName.equals(key)) {
						mIsEnabled = PreferenceManager
								...
								.getBoolean(mPreferenceName, true);
					}
				}
			};
