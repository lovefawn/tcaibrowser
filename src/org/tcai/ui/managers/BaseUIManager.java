...
import org.tcai.ui.dialogs.GeolocationPermissionsDialog;
import org.tcai.ui.fragments.BaseWebViewFragment;
import org.tcai.ui.fragments.StartPageFragment;
...
public abstract class BaseUIManager implements UIManager {// ,
	...
	protected TcaiBrowserActivity mActivity;
	...
	public BaseUIManager(TcaiBrowserActivity activity) {
		mActivity = activity;
	...
	protected abstract BaseWebViewFragment getWebViewFragmentByUUID(
			UUID fragmentId);
	...
	protected void setApplicationButtonImage(Bitmap icon) {
		BitmapDrawable image = ApplicationUtils.getApplicationButtonImage(
				mActivity, icon);
	...
	public void addTab(boolean loadHomePage, boolean privateBrowsing) {
		if (loadHomePage) {
			addTab(PreferenceManager.getDefaultSharedPreferences(mActivity)
					.getString(Constants.PREFERENCE_HOME_PAGE,
							Constants.URL_ABOUT_START), false, privateBrowsing);
		} else {
	...
	@Override
	public void loadUrl(UUID tabId, String url,
			boolean loadInCurrentTabIfNotFound) {
	...
	@Override
	public void loadRawUrl(UUID tabId, String url,
			boolean loadInCurrentTabIfNotFound) {
		BaseWebViewFragment fragment = getWebViewFragmentByUUID(tabId);
		if (fragment != null) {
	...
	public void loadHomePage() {
		...
		loadUrl(PreferenceManager.getDefaultSharedPreferences(mActivity)
				.getString(Constants.PREFERENCE_HOME_PAGE,
	...
	public void openBookmarksActivityForResult() {
		...
		mActivity.startActivityForResult(i,
				TcaiBrowserActivity.ACTIVITY_BOOKMARKS);
	}
	...
	public void shareCurrentPage() {
		...
		if (webView != null) {
			ApplicationUtils.sharePage(mActivity, webView.getTitle(),
					webView.getUrl());
		}
	}
	...
	public void setHttpAuthUsernamePassword(String host, String realm,
		...
		getCurrentWebView().setHttpAuthUsernamePassword(host, realm, username,
				password);
	}
		...
		if (intent != null) {
			if (Intent.ACTION_VIEW.equals(intent.getAction())
				...
				if (!TextUtils.isEmpty(url)) {
					...
					// We do not have an url. Open a new tab if there is no tab
					// currently opened,
					// else do nothing.
					if (getTabCount() <= 0) {
						addTab(true, false);
			...
			} else if (Constants.ACTION_BROWSER_CONTEXT_MENU.equals(intent
					.getAction())) {
				if (intent.hasExtra(Constants.EXTRA_ACTION_ID)) {
					int actionId = intent.getIntExtra(
							Constants.EXTRA_ACTION_ID, -1);

					switch (actionId) {
					case TcaiBrowserActivity.CONTEXT_MENU_OPEN:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(TcaiBrowserActivity.CONTEXT_MENU_OPEN);
						} else {
							loadUrl(intent.getStringExtra(Constants.EXTRA_URL));
						}
						break;

					case TcaiBrowserActivity.CONTEXT_MENU_OPEN_IN_NEW_TAB:

						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(
									TcaiBrowserActivity.CONTEXT_MENU_OPEN_IN_NEW_TAB,
									intent.getBooleanExtra(
											Constants.EXTRA_INCOGNITO, false));
						} else {
					...
					case TcaiBrowserActivity.CONTEXT_MENU_OPEN_IN_BACKGROUND:

						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(
									TcaiBrowserActivity.CONTEXT_MENU_OPEN_IN_BACKGROUND,
									intent.getBooleanExtra(
											Constants.EXTRA_INCOGNITO, false));
						} else {
							addTab(intent.getStringExtra(Constants.EXTRA_URL),
									true, intent.getBooleanExtra(
											Constants.EXTRA_INCOGNITO, false));
						}
						break;

					case TcaiBrowserActivity.CONTEXT_MENU_COPY:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(TcaiBrowserActivity.CONTEXT_MENU_COPY);
						} else {
							ApplicationUtils.copyTextToClipboard(
									mActivity,
									intent.getStringExtra(Constants.EXTRA_URL),
									mActivity.getResources().getString(
											R.string.UrlCopyToastMessage));
						}
						break;

					case TcaiBrowserActivity.CONTEXT_MENU_DOWNLOAD:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(TcaiBrowserActivity.CONTEXT_MENU_DOWNLOAD);
						} else {
							DownloadItem item = new DownloadItem(
									intent.getStringExtra(Constants.EXTRA_URL));

							long id = ((DownloadManager) mActivity
									.getSystemService(Context.DOWNLOAD_SERVICE))
									.enqueue(item);
							item.setId(id);

							Controller.getInstance().getDownloadsList()
									.add(item);
					...
					case TcaiBrowserActivity.CONTEXT_MENU_SHARE:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
								.getIntExtra(Constants.EXTRA_HIT_TEST_RESULT,
										-1)) {
							requestHrefNode(TcaiBrowserActivity.CONTEXT_MENU_SHARE);
						} else {
							ApplicationUtils.sharePage(mActivity, null,
									intent.getStringExtra(Constants.EXTRA_URL));
						}
						break;
					...
					default:
						if (HitTestResult.SRC_IMAGE_ANCHOR_TYPE == intent
							...
							Controller
									.getInstance()
									.getAddonManager()
									.onContributedContextLinkMenuItemSelected(
											mActivity,
											actionId,
											intent.getIntExtra(
													Constants.EXTRA_HIT_TEST_RESULT,
													-1),
											intent.getStringExtra(Constants.EXTRA_URL),
											getCurrentWebView());
						}
	...
	public void onPageFinished(final WebView view, final String url) {
		...
		view.postDelayed(new Runnable() {
			...
			@Override
			public void run() {
				if (BookmarksWrapper.urlHasBookmark(
						mActivity.getContentResolver(), url,
						view.getOriginalUrl())) {
					...
					new ThumbnailSaver(
							...
							p,
							ApplicationUtils
									.getBookmarksThumbnailsDimensions(mActivity))
							.execute();
				}
			}
		}, 2000);
	...
	public void onGeolocationPermissionsShowPrompt(String origin,
		...
		if (mGeolocationPermissionsDialog == null) {
			mGeolocationPermissionsDialog = new GeolocationPermissionsDialog(
					mActivity);
		}
	...
	public void saveTabs() {
		String userStartPage = PreferenceManager.getDefaultSharedPreferences(
				mActivity).getString(Constants.PREFERENCE_HOME_PAGE,
				Constants.URL_ABOUT_START);

		Set<String> tabs = new HashSet<String>();

		for (BaseWebViewFragment f : getTabsFragments()) {
			if (!f.isStartPageShown() && !f.isWebViewOnUrl(userStartPage)
				...
				case FOCUS_NODE_HREF:
					...
					case TcaiBrowserActivity.CONTEXT_MENU_DOWNLOAD:
						...
						Toast.makeText(
								...
								String.format(mActivity
										.getString(R.string.DownloadStart),
										item.getFileName()), Toast.LENGTH_SHORT)
								.show();
						break;
	...
	private void requestHrefNode(int action, boolean incognito) {
		...
		if (webView != null) {
			...
			final Message msg = mHandler.obtainMessage(FOCUS_NODE_HREF, action,
					incognito ? 1 : 0, hrefMap);
	...
	/**
	 * Check if the current tab can be reused to display an intent request. A
	 * tab is reusable if it is on the user-defined start page.
	 * 
	 * @return True if the current tab can be reused.
	 */
	private boolean isCurrentTabReusable() {
		String homePageUrl = PreferenceManager.getDefaultSharedPreferences(
				mActivity).getString(Constants.PREFERENCE_HOME_PAGE,
				Constants.URL_ABOUT_START);
		BaseWebViewFragment currentWebViewFragment = getCurrentWebViewFragment();
		CustomWebView currentWebView = getCurrentWebView();

		return (currentWebViewFragment != null && currentWebViewFragment
				.isStartPageShown())
				|| (currentWebView != null && homePageUrl != null && homePageUrl
						.equals(currentWebView.getUrl()));
	}
