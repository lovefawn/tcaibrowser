...
public class PanelLayout extends RelativeLayout {
	...
	public PanelLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		if (!isInEditMode()) {
			...
			if (mPreferences.getString(
					Constants.PREFERENCE_NEWPHONE_LAYOUT_TYPE,
					Constants.PREFERENCE_NEWPHONE_LAYOUT_BOTTOM).equals(
					Constants.PREFERENCE_NEWPHONE_LAYOUT_TOP))
				v = layoutInflater.inflate(R.layout.panel_layout_top, this);
			else
				v = layoutInflater.inflate(R.layout.panel_layout_bottom, this);
		...
		case MotionEvent.ACTION_DOWN:
			if (mPreferences.getString(
				...
				if (y > mBezelTopDelta) {
					...
					if ((x >= mTranslation) && (x <= mTranslation + bezelSize)) {
						return true;
					}
				}
			}
				...
				if (y < mPanel.getHeight() - mBezelTopDelta) {
					float x = ev.getX();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (y < mPanel.getHeight() - mBezelTopDelta) {
				...
				float bezelSize;
				if (mPanelShown) {
					bezelSize = mBezelSizeOpened; // ÓÒ²àÏÔÊ¾ÇøÓò
				} else {
					float height = mPanel.getHeight() - mBezelTopDelta;
					if ((y - mBezelTopDelta <= 0.1 * height)
							|| (y - mBezelTopDelta >= 0.9 * height)) {
	...
	public void showPanel() {
		...
		AnimatorSet.Builder b = animator.play(ObjectAnimator.ofFloat(mPanel,
				"alpha", 1));

		b.with(ObjectAnimator.ofFloat(mContent, "translationX",
				mPanel.getWidth()));
		...
		mAnimator
				.setDuration((long) (ANIMATION_DURATION * ((mPanel.getWidth() - mTranslation) / mPanel
						.getWidth())));
	...
	public void hidePanel() {
		...
		mAnimator
				.setDuration((long) (ANIMATION_DURATION * (mTranslation / mPanel
						.getWidth())));
		mAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
