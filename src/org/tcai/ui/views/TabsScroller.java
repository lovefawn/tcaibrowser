...
public class TabsScroller extends ScrollerView {
	...
	void handleDataChanged(int newscroll) {
		...
		for (int i = 0; i < mAdapter.getCount(); i++) {
			...
			lp.gravity = (mHorizontal ? Gravity.CENTER_VERTICAL
	...
	private void animateOut(final View v, float velocity, float start) {
		...
		ObjectAnimator alpha = ObjectAnimator.ofFloat(v, ALPHA,
				getAlpha(v, start), getAlpha(v, target));
	...
	protected void onOrthoDragFinished(View downView) {
		...
		if (mIsOrthoDragged && downView != null) {
			// offset
			float diff = mHorizontal ? downView.getTranslationY() : downView
					.getTranslationX();
			if (Math.abs(diff) > (mHorizontal ? downView.getHeight() : downView
					.getWidth()) / 2) {
				// remove it
	...
	private float getAlpha(View v, float distance) {
		return 1 - Math.abs(distance)
				/ (mHorizontal ? v.getHeight() : v.getWidth());
	}
