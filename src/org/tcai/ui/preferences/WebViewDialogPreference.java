	...
	private void init(AttributeSet attrs) {
		if (attrs != null) {
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.WebViewDialogPreference);

			int id = a.getResourceId(R.styleable.WebViewDialogPreference_html,
					-1);
			if (id != -1) {
				mHtml = ApplicationUtils.getStringFromRawResource(getContext(),
						id);
			} else {
