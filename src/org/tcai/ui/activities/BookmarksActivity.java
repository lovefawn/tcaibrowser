...
package org.tcai.ui.activities;
...
import org.tcai.tasks.HistoryBookmarksExportTask;
import org.tcai.tasks.HistoryBookmarksImportTask;
import org.tcai.ui.fragments.BookmarksFragment;
import org.tcai.ui.fragments.HistoryFragment;
import org.tcai.ui.managers.UIManager;
import org.tcai.ui.preferences.IHistoryBookmaksExportListener;
import org.tcai.ui.preferences.IHistoryBookmaksImportListener;
import org.tcai.ui.tabs.GenericTabListener;
...
public class BookmarksActivity extends StatActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tab.setTabListener(new GenericTabListener<BookmarksFragment>(this,
				"bookmarks", BookmarksFragment.class));
		actionBar.addTab(tab);

		tab = actionBar.newTab();
		tab.setText(R.string.HistoryTabTitle);
		tab.setTabListener(new GenericTabListener<HistoryFragment>(this,
		...
		if ((savedInstanceState != null)
			...
			int selectedIndex = savedInstanceState
					.getInt(EXTRA_SELECTED_TAB_INDEX);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putInt(EXTRA_SELECTED_TAB_INDEX, getActionBar()
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (getActionBar().getSelectedNavigationIndex() == 0) {
			menu.findItem(R.id.BookmarksActivityMenuSortBookmarks).setVisible(
					true);
		} else {
			menu.findItem(R.id.BookmarksActivityMenuSortBookmarks).setVisible(
					false);
		...
		List<AddonMenuItem> contributedMenuItems = Controller
				.getInstance()
				.getAddonManager()
				.getContributedHistoryBookmarksMenuItems(
						mUIManager.getCurrentWebView());
		for (AddonMenuItem item : contributedMenuItems) {
			menu.add(R.id.BookmarksActivity_AddonsMenuGroup, item.getAddon()
					.getMenuId(), 0, item.getMenuItem());
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		default:
			if (Controller
					.getInstance()
					.getAddonManager()
					.onContributedHistoryBookmarksMenuItemSelected(this,
							item.getItemId(), mUIManager.getCurrentWebView())) {
				return true;
			} else {
	...
	public void onExportProgress(int step, int progress, int total) {
		...
		case 0:
			mProgress
					.setMessage(getString(R.string.HistoryBookmarksExportCheckCardMessage));
			break;
		case 1:
			mProgress.setMessage(String.format(
					getString(R.string.HistoryBookmarksExportProgressMessage),
					progress, total));
			break;
		default:
			break;
		...
		if (message != null) {
			ApplicationUtils
					.showErrorDialog(
							...
							String.format(
									getString(R.string.HistoryBookmarksExportErrorMessage),
									message));
		}
	}
	...
	public void onImportProgress(int step, int progress, int total) {
		...
		case 0:
			mProgress
					.setMessage(getString(R.string.HistoryBookmarksImportReadingFile));
			break;
		case 1:
			mProgress
					.setMessage(getString(R.string.HistoryBookmarksImportParsingFile));
			break;
		case 2:
			mProgress.setMessage(String.format(
					getString(R.string.HistoryBookmarksImportProgressMessage),
					progress, total));
			break;
		case 3:
			mProgress
					.setMessage(String
							.format(getString(R.string.HistoryBookmarksImportFoldersProgressMessage),
									progress, total));
			break;
		case 4:
			mProgress
					.setMessage(getString(R.string.HistoryBookmarksImportFoldersLinkMessage));
			break;
		case 5:
			mProgress
					.setMessage(String
							.format(getString(R.string.HistoryBookmarksImportBookmarksProgressMessage),
									progress, total));
			break;
		case 6:
			mProgress
					.setMessage(String
							.format(getString(R.string.HistoryBookmarksImportHistoryProgressMessage),
									progress, total));
			break;
		case 7:
			mProgress
					.setMessage(getString(R.string.HistoryBookmarksImportInsertMessage));
			break;
		default:
			break;
		}
		...
		if (message != null) {
			ApplicationUtils
					.showErrorDialog(
							...
							String.format(
									getString(R.string.HistoryBookmarksImportErrorMessage),
									message));
		}
	}
	...
	private void importHistoryBookmarks() {
		...
		builder.setTitle(getResources().getString(
				R.string.HistoryBookmarksImportSourceTitle));
		builder.setSingleChoiceItems(choices, 0, new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				mImportTask = new HistoryBookmarksImportTask(
						BookmarksActivity.this, BookmarksActivity.this);

				mProgress = ProgressDialog
						.show(BookmarksActivity.this,
				...
				boolean retVal = mImportSyncThread.compareAndSet(null,
						mImportTask);
				if (retVal) {
	...
	private void exportHistoryBookmarks() {
		...
		mProgress = ProgressDialog.show(this,
				...
				getString(R.string.HistoryBookmarksExportInitialMessage), true,
				false);
		...
		if (retVal) {
			mExportTask.execute(BookmarksWrapper
					.getAllHistoryBookmarks(getContentResolver()));
		}
	}
	...
	private void clearHistoryBookmarks() {
		...
		builder.setIcon(android.R.drawable.ic_dialog_info);
		builder.setTitle(getResources().getString(
				R.string.HistoryBookmarksClearTitle));
		builder.setSingleChoiceItems(
				getResources().getStringArray(
						R.array.ClearHistoryBookmarksChoice), 0,
				new OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						switch (which) {
						case 0:
							BookmarksWrapper.clearHistoryAndOrBookmarks(
									getContentResolver(), true, false);
							break;

						case 1:
							BookmarksWrapper.clearHistoryAndOrBookmarks(
									getContentResolver(), false, true);
							break;

						case 2:
							BookmarksWrapper.clearHistoryAndOrBookmarks(
									getContentResolver(), true, true);
							break;
					...
					public void onClick(DialogInterface dialog, int which) {
						Editor editor = PreferenceManager
								.getDefaultSharedPreferences(
										BookmarksActivity.this).edit();
						editor.putInt(Constants.PREFERENCE_BOOKMARKS_SORT_MODE,
								which);
						editor.commit();
