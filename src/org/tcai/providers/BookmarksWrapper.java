...
public class BookmarksWrapper {
	...
	public static String[] HISTORY_BOOKMARKS_PROJECTION = new String[] {
			BookmarksProvider.Columns._ID, BookmarksProvider.Columns.TITLE,
			BookmarksProvider.Columns.URL, BookmarksProvider.Columns.VISITS,
	...
	public static CursorLoader getCursorLoaderForStartPage(Context context,
			int limit) {
		...
		return new CursorLoader(context, BookmarksProvider.BOOKMARKS_URI,
				HISTORY_BOOKMARKS_PROJECTION, whereClause, null, orderClause);
	}

	public static CursorLoader getCursorLoaderForBookmarks(Context context,
			long parentFolderId) {
		int sortMode = PreferenceManager.getDefaultSharedPreferences(context)
				.getInt(Constants.PREFERENCE_BOOKMARKS_SORT_MODE, 0);

		String whereClause = BookmarksProvider.Columns.PARENT_FOLDER_ID + " = "
				+ parentFolderId + " AND ("
				+ BookmarksProvider.Columns.BOOKMARK + " = 1 OR "
				+ BookmarksProvider.Columns.IS_FOLDER + " = 1)";

		String orderClause;
		switch (sortMode) {
		case 0:
			orderClause = BookmarksProvider.Columns.IS_FOLDER + " DESC, "
	...
	public static CursorLoader getCursorLoaderForHistory(Context context) {
		String whereClause = BookmarksProvider.Columns.VISITS + " > 0 AND "
	...
	public static Cursor getAllHistoryBookmarks(ContentResolver contentResolver) {
		return contentResolver.query(BookmarksProvider.BOOKMARKS_URI,
				HISTORY_BOOKMARKS_PROJECTION, null, null, null);
	}

	public static BookmarkHistoryItem getBookmarkById(
		...
		Cursor c = contentResolver.query(BookmarksProvider.BOOKMARKS_URI,
				HISTORY_BOOKMARKS_PROJECTION, whereClause, null, null);
		if (c != null) {
			if (c.moveToFirst()) {
				String title = c.getString(c
						.getColumnIndex(BookmarksProvider.Columns.TITLE));
				String url = c.getString(c
						.getColumnIndex(BookmarksProvider.Columns.URL));
				boolean isBookmarks = c.getInt(c
						.getColumnIndex(BookmarksProvider.Columns.BOOKMARK)) > 0 ? true
						: false;
				boolean isFolder = c.getInt(c
						.getColumnIndex(BookmarksProvider.Columns.IS_FOLDER)) > 0 ? true
						: false;
				long folderId = c
						.getLong(c
								.getColumnIndex(BookmarksProvider.Columns.PARENT_FOLDER_ID));
				byte[] favIcon = c.getBlob(c
						.getColumnIndex(BookmarksProvider.Columns.FAVICON));
				result = new BookmarkHistoryItem(id, title, url, isBookmarks,
						isFolder, folderId, favIcon);
			}
	...
	public static void clearHistoryAndOrBookmarks(
		...
		if (clearHistory && clearBookmarks) {
			...
			whereClause = "((" + BookmarksProvider.Columns.BOOKMARK
					+ " = 0) OR (" + BookmarksProvider.Columns.BOOKMARK
					+ " IS NULL)) AND (" + BookmarksProvider.Columns.IS_FOLDER
					+ " = 0)";
		} else if (clearBookmarks) {
			whereClause = "(" + BookmarksProvider.Columns.BOOKMARK
					+ " = 1) OR (" + BookmarksProvider.Columns.IS_FOLDER
					+ " = 1)";
		}

		contentResolver.delete(BookmarksProvider.BOOKMARKS_URI, whereClause,
				null);
	}

	public static List<FolderItem> getFirstLevelFoldersList(
			ContentResolver contentResolver) {
		List<FolderItem> result = new ArrayList<FolderItem>();

		String whereClause = BookmarksProvider.Columns.IS_FOLDER + " = 1 AND "
				+ BookmarksProvider.Columns.PARENT_FOLDER_ID + " = -1";
		...
		if ((c != null) && (c.moveToFirst())) {
			...
			do {
				result.add(new FolderItem(c.getLong(idIndex), c
	...
	public static long getFolderId(ContentResolver contentResolver,
			String folderName, boolean createIfNotPresent) {
		String escapedFolderName = DatabaseUtils.sqlEscapeString(folderName);

		String whereClause = BookmarksProvider.Columns.TITLE + " = "
				+ escapedFolderName + " AND "
				+ BookmarksProvider.Columns.IS_FOLDER + " = 1";
		...
		if ((c != null) && (c.moveToFirst())) {
			...
			if (createIfNotPresent) {
				...
				Uri result = contentResolver.insert(
						BookmarksProvider.BOOKMARKS_URI, values);

				Cursor inserted = contentResolver.query(result,
						HISTORY_BOOKMARKS_PROJECTION, null, null, null);
				if ((inserted != null) && (inserted.moveToFirst())) {
					return inserted.getLong(inserted
							.getColumnIndex(BookmarksProvider.Columns._ID));
				} else {
					return -1;
				}
	...
	public static void setAsBookmark(ContentResolver contentResolver, long id,
			long folderId, String title, String url, boolean isBookmark) {
		...
		if (id != -1) {
			...
			Cursor cursor = contentResolver.query(
					BookmarksProvider.BOOKMARKS_URI, colums, whereClause, null,
					null);
		...
		if (isBookmark) {
			...
			values.put(BookmarksProvider.Columns.CREATION_DATE,
					new Date().getTime());
		} else {
			values.put(BookmarksProvider.Columns.BOOKMARK, 0);
		...
		if (bookmarkExist) {
			contentResolver.update(BookmarksProvider.BOOKMARKS_URI, values,
					BookmarksProvider.Columns._ID + " = " + id, null);
		} else {
	...
	public static void deleteBookmark(ContentResolver contentResolver, long id) {
		...
		if (c != null) {
			if (c.moveToFirst()) {
				if (c.getInt(c
						.getColumnIndex(BookmarksProvider.Columns.BOOKMARK)) == 1) {
					if (c.getInt(c
							.getColumnIndex(BookmarksProvider.Columns.VISITS)) > 0) {

						// If this record has been visited, keep it in history,
						// but remove its bookmark flag.
						ContentValues values = new ContentValues();
						values.put(BookmarksProvider.Columns.BOOKMARK, 0);
						values.put(BookmarksProvider.Columns.PARENT_FOLDER_ID,
								-1);
						values.putNull(BookmarksProvider.Columns.CREATION_DATE);

						contentResolver.update(BookmarksProvider.BOOKMARKS_URI,
								values, whereClause, null);

					} else {
						// never visited, it can be deleted.
						contentResolver.delete(BookmarksProvider.BOOKMARKS_URI,
								whereClause, null);
					}
				}
			}
	...
	public static void deleteFolder(ContentResolver contentResolver, long id) {
		...
		BookmarksProvider provider = (BookmarksProvider) contentResolver
				.acquireContentProviderClient(BookmarksProvider.BOOKMARKS_URI)
		...
		String whereClause = BookmarksProvider.Columns.PARENT_FOLDER_ID + " = "
				+ id + " AND " + BookmarksProvider.Columns.BOOKMARK + " > 0";
		c = contentResolver.query(BookmarksProvider.BOOKMARKS_URI,
				HISTORY_BOOKMARKS_PROJECTION, whereClause, null, null);
		if (c != null) {
			if (c.moveToFirst()) {
				...
				int idIndex = c.getColumnIndex(BookmarksProvider.Columns._ID);
				int visitsIndex = c
						.getColumnIndex(BookmarksProvider.Columns.VISITS);
				...
				do {
					...
					if (c.getInt(visitsIndex) > 0) {
						// If this record has been visited, keep it in history,
						// but remove its bookmark flag and its folder id.
						ContentValues values = new ContentValues();
						values.put(BookmarksProvider.Columns.BOOKMARK, 0);
						values.put(BookmarksProvider.Columns.PARENT_FOLDER_ID,
								-1);
						...
						contentResolver.update(BookmarksProvider.BOOKMARKS_URI,
								values, BookmarksProvider.Columns._ID + " = "
										+ bookmarkId, null);
					} else {
						contentResolver.delete(BookmarksProvider.BOOKMARKS_URI,
	...
	public static void deleteHistoryRecord(ContentResolver contentResolver,
		...
		if (c != null) {
			if (c.moveToFirst()) {
				if (c.getInt(c
						.getColumnIndex(BookmarksProvider.Columns.BOOKMARK)) > 0) {
					// This is a bookmark, we cannot delete it. Instead, set
					// visits count to 0 and visited date to null.
					ContentValues values = new ContentValues();
	...
	public static void updateHistory(ContentResolver contentResolver,
		...
		String escapedUrl = url != null ? DatabaseUtils.sqlEscapeString(url)
				: "";
		String escapedOriginalUrl = originalUrl != null ? DatabaseUtils
				.sqlEscapeString(originalUrl) : "";

		String whereClause = BookmarksProvider.Columns.URL + " = " + escapedUrl
				+ " OR " + BookmarksProvider.Columns.URL + " = "
				+ escapedOriginalUrl;

		Cursor cursor = contentResolver.query(BookmarksProvider.BOOKMARKS_URI,
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				...
				int visits = cursor.getInt(cursor
						.getColumnIndex(BookmarksProvider.Columns.VISITS)) + 1;
				...
				values.put(BookmarksProvider.Columns.VISITED_DATE,
						new Date().getTime());
	...
	private static final String TRUNCATE_HISTORY_DELETE_WHERE_PATTERN = "("
			+ BookmarksProvider.Columns.BOOKMARK + " = 0 OR "
			+ BookmarksProvider.Columns.BOOKMARK + " IS NULL) AND "
			+ BookmarksProvider.Columns.VISITED_DATE + " < %s";
	private static final String TRUNCATE_HISTORY_UPDATE_WHERE_PATTERN = BookmarksProvider.Columns.BOOKMARK
			+ " = 1  AND " + BookmarksProvider.Columns.VISITED_DATE + " < %s";

	/**
	 * Remove from history values prior to now minus the number of days defined
	 * in preferences. Only delete history items. For bookmarks, reset their
	 * visited value date and visits count.
	 * 
	 * @param contentResolver
	 *            The content resolver.
	 */
	public static void truncateHistory(ContentResolver contentResolver,
		...
		String whereClauseDelete = String.format(
				TRUNCATE_HISTORY_DELETE_WHERE_PATTERN, c.getTimeInMillis());
		String whereClauseUpdate = String.format(
				TRUNCATE_HISTORY_UPDATE_WHERE_PATTERN, c.getTimeInMillis());
		...
		try {
			contentResolver.delete(BookmarksProvider.BOOKMARKS_URI,
					whereClauseDelete, null);
			contentResolver.update(BookmarksProvider.BOOKMARKS_URI,
					updateValues, whereClauseUpdate, null);
		} catch (Exception e) {
			e.printStackTrace();
			Log.w("BookmarksWrapper",
					"Unable to truncate history: " + e.getMessage());
		}
	}

	private static final String UPDATE_FAVICON_WHERE_PATTERN_1 = BookmarksProvider.Columns.URL
			+ " = %s OR " + BookmarksProvider.Columns.URL + " = %s";
	 ...
	 *            The favicon.
	 */
	public static void updateFavicon(ContentResolver contentResolver,
			String url, String originalUrl, Bitmap favicon) {
		if ((url != null) && (favicon != null) && (contentResolver != null)) {
			...
			if ((originalUrl != null) && !url.equals(originalUrl)) {
				...
				whereClause = String.format(UPDATE_FAVICON_WHERE_PATTERN_1,
						url, originalUrl);
			} else {
				url = DatabaseUtils.sqlEscapeString(url);
				whereClause = String
						.format(UPDATE_FAVICON_WHERE_PATTERN_2, url);
			}
	...
	private static final String BOOKMARK_BY_URL_WHERE_PATTERN_1 = "("
			+ BookmarksProvider.Columns.URL + " = %s OR "
			+ BookmarksProvider.Columns.URL + " = %s) AND "
			+ BookmarksProvider.Columns.BOOKMARK + " = 1";
	private static final String BOOKMARK_BY_URL_WHERE_PATTERN_2 = BookmarksProvider.Columns.URL
			+ " = %s AND " + BookmarksProvider.Columns.BOOKMARK + " = 1";

	public static void updateThumbnail(ContentResolver contentResolver,
			String url, String originalUrl, Bitmap thumbnail) {
		if ((url != null) && (thumbnail != null) && (contentResolver != null)) {
			...
			if ((originalUrl != null) && !url.equals(originalUrl)) {
				...
				whereClause = String.format(BOOKMARK_BY_URL_WHERE_PATTERN_1,
						url, originalUrl);
			} else {
				url = DatabaseUtils.sqlEscapeString(url);
				whereClause = String.format(BOOKMARK_BY_URL_WHERE_PATTERN_2,
						url);
			}
			...
			try {
				...
				e.printStackTrace();
				Log.w("BookmarksWrapper",
						"Unable to update thumbnail: " + e.getMessage());
			}
		}
	}

	public static boolean urlHasBookmark(ContentResolver contentResolver,
	...
	public static void insertRawRecord(ContentResolver contentResolver,
			String title, String url, int visits, long visitedDate,
	...
	public static void fillDefaultBookmaks(ContentResolver contentResolver,
			String[] titles, String[] urls) {
		int size = Math.min(titles.length, urls.length);
		long currentDate = new Date().getTime();

		for (int i = 0; i < size; i++) {
			insertRawRecord(contentResolver, titles[i], urls[i], 2,
					currentDate, currentDate, 1);
		}
	}

	private static final String SUGGESTIONS_PATTERN = "%%%s%%";
	private static final String SUGGESTIONS_WHERE_PATTERN = BookmarksProvider.Columns.TITLE
			+ " LIKE %s OR " + BookmarksProvider.Columns.URL + " LIKE %s";
	private static final String SUGGESTIONS_ORDER = BookmarksProvider.Columns.VISITED_DATE
			+ " DESC, "
			+ BookmarksProvider.Columns.BOOKMARK
			+ " DESC, "
	 ...
	 * and bookmarks, on title and url.
	 * 
	 * @param contentResolver
	 *            The content resolver.
	 * @param pattern
	...
	public static Cursor getUrlSuggestions(ContentResolver contentResolver,
		...
		if ((pattern != null) && (pattern.length() > 0)) {
			...
			String sqlPattern = DatabaseUtils.sqlEscapeString(String.format(
					SUGGESTIONS_PATTERN, pattern));
			String whereClause = String.format(SUGGESTIONS_WHERE_PATTERN,
	...
	private static final String CHILDREN_FOLDERS_WHERE_PATTERN = BookmarksProvider.Columns.IS_FOLDER
			+ " > 0 AND "
			+ BookmarksProvider.Columns.PARENT_FOLDER_ID
			+ " = %S";

	private static Cursor getChildrenFolders(ContentResolver contentResolver,
			long folderId) {
		String whereClause = String.format(CHILDREN_FOLDERS_WHERE_PATTERN,
				folderId);
		return contentResolver.query(BookmarksProvider.BOOKMARKS_URI,
				HISTORY_BOOKMARKS_PROJECTION, whereClause, null, null);
	}
