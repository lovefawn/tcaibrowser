...
public class SslExceptionsWrapper {
	...
	// Redefinition of SSL errors constants, because those in Android SslError
	// cannot be used for bitmasks.
	...
	public static CursorLoader getSslErrorAuthoritiesCursorLoader(
			Context context) {
		return new CursorLoader(context,
				SslExceptionsProvider.SSL_EXCEPTIONS_URI,
				SSL_EXCEPTIONS_PROJECTION, null, null, null);
	}

	public static int getStatusForAuthority(ContentResolver contentResolver,
			String authority) {
		int result = AUTHORITY_UNKNOWN;

		String whereClause = SslExceptionsProvider.Columns.AUTHORITY + " = \""
				+ authority + "\"";

		Cursor c = contentResolver.query(
				SslExceptionsProvider.SSL_EXCEPTIONS_URI,
				SSL_EXCEPTIONS_PROJECTION, whereClause, null, null);
		if (c != null) {
			if (c.moveToFirst()) {
				if (c.getInt(c
						.getColumnIndex(SslExceptionsProvider.Columns.ALLOW)) > 0) {
	...
	public static void setSslException(ContentResolver contentResolver,
			String authority, int reason, boolean allow) {
		...
		if (id != -1) {
			...
			contentResolver.update(SslExceptionsProvider.SSL_EXCEPTIONS_URI,
			...
			contentResolver.insert(SslExceptionsProvider.SSL_EXCEPTIONS_URI,
					values);
		}
	}

	public static void toggleSslException(ContentResolver contentResolver,
			long id, boolean allow) {
		String whereClause = SslExceptionsProvider.Columns._ID + " = " + id;
	...
	public static void removeSslException(ContentResolver contentResolver,
			long id) {
		String whereClause = SslExceptionsProvider.Columns._ID + " = " + id;

		contentResolver.delete(SslExceptionsProvider.SSL_EXCEPTIONS_URI,
				whereClause, null);
	}
	...
	private static long getIdForAuthority(ContentResolver contentResolver,
			String authority) {
		long result = -1;
		...
		if (c != null) {
			if (c.moveToFirst()) {
				result = c.getLong(c
						.getColumnIndex(SslExceptionsProvider.Columns._ID));
			}
