...
public class BookmarksAdapter extends SimpleCursorAdapter {
	...
	public BookmarksAdapter(Context context, int layout, Cursor c,
			String[] from, int[] to, int defaultThumbnailId) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		boolean isFolder = getCursor()
				.getInt(getCursor().getColumnIndex(
						BookmarksProvider.Columns.IS_FOLDER)) > 0 ? true
				: false;

		if (!isFolder) {
			byte[] thumbnail = getCursor().getBlob(
					getCursor().getColumnIndex(
							BookmarksProvider.Columns.THUMBNAIL));
			if (thumbnail != null) {
				thumbnailView.setImageBitmap(BitmapFactory.decodeByteArray(
						thumbnail, 0, thumbnail.length));
			} else {
